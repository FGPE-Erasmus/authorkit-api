...
export class InstructionService {
    ...
    constructor(
        ...
        @InjectQueue(INSTRUCTION_SYNC_QUEUE)
        private readonly instructionSyncQueue: Queue,
    ...
    public async createOne(
        user: UserEntity,
        dto: InstructionEntity,
        file: any
    ): Promise<InstructionEntity> {
        dto.pathname = file.originalname;
        try {
            const entity = await this.repository.save(
                plainToClass(InstructionEntity, dto)
            );
            this.instructionSyncQueue.add(INSTRUCTION_SYNC_CREATE, {
                user,
                entity
            });
    ...
    public async updateOne(
        ...
        try {
            ...
            this.instructionSyncQueue.add(INSTRUCTION_SYNC_UPDATE, {
                user,
                entity
            });
            this.instructionSyncQueue.add(
    ...
    public async deleteOne(
        ...
        this.instructionSyncQueue.add(INSTRUCTION_SYNC_DELETE, {
            user,
            entity
        });
        return entity;
    ...
    public async translate(
        ...
        if (
            instruction.format !== TextFormat.TXT &&
            instruction.format !== TextFormat.HTML &&
            instruction.format !== TextFormat.MARKDOWN
        ) {
            throw new BadRequestException(
                `Cannot translate instructions in ${instruction.format}`
            );
        }
        let contents = Buffer.from(
            await this.getContents(user, id),
            'base64'
        ).toString('utf8');
        if (
            instruction.nat_lang !== nat_lang &&
            languages[nat_lang] &&
            languages[instruction.nat_lang]
        ) {
            const translated = await translate(contents, {
                from: instruction.nat_lang,
                to: nat_lang
            });
            ...
            } as InstructionEntity,
            {
                originalname:
                    ...
                    (instruction.pathname.startsWith(`${instruction.nat_lang}_`)
                        ? instruction.pathname.substr(
                              instruction.nat_lang.length + 1
                          )
