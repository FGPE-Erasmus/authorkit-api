...
export class PermissionService extends TypeOrmCrudService<PermissionEntity> {
    ...
    public async findAllPermissionsOf(
        user_id: string
    ): Promise<DeepPartial<PermissionEntity>[]> {
        const permissions: DeepPartial<PermissionEntity>[] = await super.find({
            where: {
                user_id
            }
        });
        const projectIds: string[] = permissions.map(
        ...
        (await this.projectService.getPublicProjects(options)).forEach(
            (project) => {
                permissions.push({
    ...
    public async findPermissionOf(
        user_id: string,
        project_id: string
    ): Promise<DeepPartial<PermissionEntity> | null> {
    ...
    public async addOwnerPermission(
        project_id: string,
        user_id: string
