...
export class StatementService {
    ...
    constructor(
        ...
        @InjectQueue(STATEMENT_SYNC_QUEUE)
        private readonly statementSyncQueue: Queue,
    ...
    public async createOne(
        ...
        try {
            const entity = await this.repository.save(
                plainToClass(StatementEntity, dto)
            );
            this.statementSyncQueue.add(STATEMENT_SYNC_CREATE, {
                user,
                entity
            });
    ...
    public async updateOne(
        ...
        try {
            ...
            this.statementSyncQueue.add(STATEMENT_SYNC_UPDATE, {
                user,
                entity
            });
            this.statementSyncQueue.add(
    ...
    public async translate(
        ...
        if (
            statement.format !== TextFormat.TXT &&
            statement.format !== TextFormat.HTML &&
            statement.format !== TextFormat.MARKDOWN
        ) {
            throw new BadRequestException(
                `Cannot translate statements in ${statement.format}`
            );
        }
            ...
            } as StatementEntity,
            {
                originalname:
                    ...
                    (statement.pathname.startsWith(`${statement.nat_lang}_`)
                        ? statement.pathname.substr(
                              statement.nat_lang.length + 1
                          )
                        : statement.pathname),
