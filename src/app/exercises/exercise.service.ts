...
export class ExerciseService extends TypeOrmCrudService<ExerciseEntity> {
    ...
    constructor(
        ...
        @InjectQueue(EXERCISE_SYNC_QUEUE)
        private readonly exerciseSyncQueue: Queue,
    ...
    public async getMany(
        req: CrudRequest
    ): Promise<GetManyDefaultResponse<ExerciseEntity> | ExerciseEntity[]> {
    ...
    public async importProcessEntries(
        ...
        const exercise = await this.importMetadataFile(
            user,
            project_id,
            root_metadata
        );

        await this.exerciseSyncQueue.add(EXERCISE_SYNC_CREATE, {
            user,
            exercise
        });
        ...
        Object.keys(result['dynamic-correctors']).forEach(
            (related_entity_key) => {
                asyncImporters.push(
                    this.dynamicCorrectorService.importProcessEntries(
                        user,
                        exercise,
                        result['dynamic-correctors'][related_entity_key]
                    )
                );
            }
        );
        ...
        Object.keys(result['embeddables']).forEach((related_entity_key) => {
            asyncImporters.push(
                this.embeddableService.importProcessEntries(
                    user,
                    exercise,
                    result['embeddables'][related_entity_key]
                )
            );
        });

        Object.keys(result['feedback-generators']).forEach(
            (related_entity_key) => {
                asyncImporters.push(
                    this.feedbackGeneratorService.importProcessEntries(
                        user,
                        exercise,
                        result['feedback-generators'][related_entity_key]
                    )
                );
            }
        );
        ...
        Object.keys(result['instructions']).forEach((related_entity_key) => {
            asyncImporters.push(
                this.instructionService.importProcessEntries(
                    user,
                    exercise,
                    result['instructions'][related_entity_key]
                )
            );
        });
        ...
        Object.keys(result['libraries']).forEach((related_entity_key) => {
            asyncImporters.push(
                this.libraryService.importProcessEntries(
                    user,
                    exercise,
                    result['libraries'][related_entity_key]
                )
            );
        });
        ...
        Object.keys(result['skeletons']).forEach((related_entity_key) => {
            asyncImporters.push(
                this.skeletonService.importProcessEntries(
                    user,
                    exercise,
                    result['skeletons'][related_entity_key]
                )
            );
        });
        ...
        Object.keys(result['solutions']).forEach((related_entity_key) => {
            asyncImporters.push(
                this.solutionService.importProcessEntries(
                    user,
                    exercise,
                    result['solutions'][related_entity_key]
                )
            );
        });
        ...
        Object.keys(result['statements']).forEach((related_entity_key) => {
            asyncImporters.push(
                this.statementService.importProcessEntries(
                    user,
                    exercise,
                    result['statements'][related_entity_key]
                )
            );
        });

        Object.keys(result['static-correctors']).forEach(
            (related_entity_key) => {
                asyncImporters.push(
                    this.staticCorrectorService.importProcessEntries(
                        user,
                        exercise,
                        result['static-correctors'][related_entity_key]
                    )
                );
            }
        );
        ...
        Object.keys(result['templates']).forEach((related_entity_key) => {
            asyncImporters.push(
                this.templateService.importProcessEntries(
                    user,
                    exercise,
                    result['templates'][related_entity_key]
                )
            );
        });
        ...
        Object.keys(result['test-generators']).forEach((related_entity_key) => {
            asyncImporters.push(
                this.testGeneratorService.importProcessEntries(
                    user,
                    exercise,
                    result['test-generators'][related_entity_key]
                )
            );
        });
        ...
        Object.keys(result['tests']).forEach((related_entity_key) => {
            asyncImporters.push(
                this.testService.importProcessEntries(
                    user,
                    exercise,
                    result['tests'][related_entity_key]
                )
            );
        });
        ...
        Object.keys(result['testsets']).forEach((related_entity_key) => {
            asyncImporters.push(
                this.testsetsService.importProcessEntries(
                    user,
                    exercise,
                    result['testsets'][related_entity_key]
                )
            );
        });
    ...
    public async importSipeSingle(
        ...
        asyncImporters.push(
            this.statementService.importProcessEntries(user, exercise, {
                'metadata.json': {
                    buffer: () =>
                        Buffer.from(
                            JSON.stringify({
        ...
        asyncImporters.push(
            this.skeletonService.importProcessEntries(user, exercise, {
                'metadata.json': {
                    buffer: () =>
                        Buffer.from(
                            JSON.stringify({
        ...
        asyncImporters.push(
            this.staticCorrectorService.importProcessEntries(user, exercise, {
                'metadata.json': {
                    buffer: () =>
                        Buffer.from(
                            JSON.stringify({
        ...
        asyncImporters.push(
            this.dynamicCorrectorService.importProcessEntries(user, exercise, {
                'metadata.json': {
                    buffer: () =>
                        Buffer.from(
                            JSON.stringify({
    ...
    public async importMef(
        ...
        if (!baseFile) {
            this.throwBadRequestException(
                'Archive misses required "Content.xml"'
            );
        }
        ...
        // create exercise
        const exercise: ExerciseEntity = await this.importMefCreateExercise(
            user,
            project_id,
            baseJson.Problem.$
        );
        ...
        if (
            ...
            entries[baseJson.Problem.$.Description]
        ) {
            asyncImporters.push(
                this.statementService.importProcessEntries(user, exercise, {
                    'metadata.json': {
        ...
        if (
            ...
            entries[baseJson.Problem.$.Environment]
        ) {
            asyncImporters.push(
                this.libraryService.importProcessEntries(user, exercise, {
                    'metadata.json': {
                        buffer: () =>
                            Buffer.from(
                                JSON.stringify({
        ...
        if (baseJson.Problem.$.Program && entries[baseJson.Problem.$.Program]) {
            asyncImporters.push(
                this.solutionService.importProcessEntries(user, exercise, {
                    'metadata.json': {
                        buffer: () =>
                            Buffer.from(
                                JSON.stringify({
        ...
        if (
            baseJson.Problem.Tests &&
            baseJson.Problem.Tests[0] &&
            baseJson.Problem.Tests[0].Test
        ) {
            baseJson.Problem.Tests[0].Test.forEach((test) => {
                const inputPath =
                    test.$['xml:id'].replace('tests.', 'tests/') +
                    '/' +
                    test.$.input;
                const outputPath =
                    test.$['xml:id'].replace('tests.', 'tests/') +
                ...
                asyncImporters.push(
                    this.testService.importProcessEntries(user, exercise, {
                        'metadata.json': {
                            buffer: () =>
                                Buffer.from(
                                    JSON.stringify({
    ...
    public async importMefCreateExercise(
        user: UserEntity,
        project_id: string,
        metadata: any
    ): Promise<ExerciseEntity> {
        ...
        for (const dynamic_corrector of exercise.dynamic_correctors) {
            ...
            asyncArchiveWriters.push(
                this.addFileFromGithubToArchive(
                    ...
                    exercise,
                    archive,
                    `${base_path}${dynamic_corrector_path}metadata.json`,
                    `${archive_base_path}${dynamic_corrector_path}metadata.json`
                ),
                this.addFileFromGithubToArchive(
                    user,
                    exercise,
                    archive,
                    `${base_path}${dynamic_corrector_path}${dynamic_corrector.pathname}`,
                    `${archive_base_path}${dynamic_corrector_path}${dynamic_corrector.pathname}`
                )
            );
        }
        ...
        for (const static_corrector of exercise.static_correctors) {
            ...
            asyncArchiveWriters.push(
                this.addFileFromGithubToArchive(
                    ...
                    exercise,
                    archive,
                    `${base_path}${static_corrector_path}metadata.json`,
                    `${archive_base_path}${static_corrector_path}metadata.json`
                ),
                this.addFileFromGithubToArchive(
                    user,
                    exercise,
                    archive,
                    `${base_path}${static_corrector_path}${static_corrector.pathname}`,
                    `${archive_base_path}${static_corrector_path}${static_corrector.pathname}`
                )
            );
        }
        ...
        for (const embeddable of exercise.embeddables) {
            ...
            asyncArchiveWriters.push(
                this.addFileFromGithubToArchive(
                    ...
                    exercise,
                    archive,
                    `${base_path}${embeddable_path}metadata.json`,
                    `${archive_base_path}${embeddable_path}metadata.json`
                ),
                this.addFileFromGithubToArchive(
                    user,
                    exercise,
                    archive,
                    `${base_path}${embeddable_path}${embeddable.pathname}`,
                    `${archive_base_path}${embeddable_path}${embeddable.pathname}`
                )
            );
        }
        ...
        for (const feedback_generator of exercise.feedback_generators) {
            ...
            asyncArchiveWriters.push(
                this.addFileFromGithubToArchive(
                    ...
                    exercise,
                    archive,
                    `${base_path}${feedback_generator_path}metadata.json`,
                    `${archive_base_path}${feedback_generator_path}metadata.json`
                ),
                this.addFileFromGithubToArchive(
                    user,
                    exercise,
                    archive,
                    `${base_path}${feedback_generator_path}${feedback_generator.pathname}`,
                    `${archive_base_path}${feedback_generator_path}${feedback_generator.pathname}`
                )
            );
        }
        ...
        for (const instruction of exercise.instructions) {
            ...
            asyncArchiveWriters.push(
                this.addFileFromGithubToArchive(
                    ...
                    exercise,
                    archive,
                    `${base_path}${instruction_path}metadata.json`,
                    `${archive_base_path}${instruction_path}metadata.json`
                ),
                this.addFileFromGithubToArchive(
                    user,
                    exercise,
                    archive,
                    `${base_path}${instruction_path}${instruction.pathname}`,
                    `${archive_base_path}${instruction_path}${instruction.pathname}`
                )
            );
        }
        ...
        for (const library of exercise.libraries) {
            ...
            asyncArchiveWriters.push(
                this.addFileFromGithubToArchive(
                    ...
                    exercise,
                    archive,
                    `${base_path}${library_path}metadata.json`,
                    `${archive_base_path}${library_path}metadata.json`
                ),
                this.addFileFromGithubToArchive(
                    user,
                    exercise,
                    archive,
                    `${base_path}${library_path}${library.pathname}`,
                    `${archive_base_path}${library_path}${library.pathname}`
                )
            );
        }
        ...
        for (const skeleton of exercise.skeletons) {
            ...
            asyncArchiveWriters.push(
                this.addFileFromGithubToArchive(
                    ...
                    exercise,
                    archive,
                    `${base_path}${skeleton_path}metadata.json`,
                    `${archive_base_path}${skeleton_path}metadata.json`
                ),
                this.addFileFromGithubToArchive(
                    user,
                    exercise,
                    archive,
                    `${base_path}${skeleton_path}${skeleton.pathname}`,
                    `${archive_base_path}${skeleton_path}${skeleton.pathname}`
                )
            );
        }
        ...
        for (const solution of exercise.solutions) {
            ...
            asyncArchiveWriters.push(
                this.addFileFromGithubToArchive(
                    ...
                    exercise,
                    archive,
                    `${base_path}${solution_path}metadata.json`,
                    `${archive_base_path}${solution_path}metadata.json`
                ),
                this.addFileFromGithubToArchive(
                    user,
                    exercise,
                    archive,
                    `${base_path}${solution_path}${solution.pathname}`,
                    `${archive_base_path}${solution_path}${solution.pathname}`
                )
            );
        }
        ...
        for (const statement of exercise.statements) {
            ...
            asyncArchiveWriters.push(
                this.addFileFromGithubToArchive(
                    ...
                    exercise,
                    archive,
                    `${base_path}${statement_path}metadata.json`,
                    `${archive_base_path}${statement_path}metadata.json`
                ),
                this.addFileFromGithubToArchive(
                    user,
                    exercise,
                    archive,
                    `${base_path}${statement_path}${statement.pathname}`,
                    `${archive_base_path}${statement_path}${statement.pathname}`
                )
            );
        }
        ...
        for (const template of exercise.templates) {
            ...
            asyncArchiveWriters.push(
                this.addFileFromGithubToArchive(
                    ...
                    exercise,
                    archive,
                    `${base_path}${template_path}metadata.json`,
                    `${archive_base_path}${template_path}metadata.json`
                ),
                this.addFileFromGithubToArchive(
                    user,
                    exercise,
                    archive,
                    `${base_path}${template_path}${template.pathname}`,
                    `${archive_base_path}${template_path}${template.pathname}`
                )
            );
        }
        ...
        for (const test_generator of exercise.test_generators) {
            ...
            asyncArchiveWriters.push(
                this.addFileFromGithubToArchive(
                    ...
                    exercise,
                    archive,
                    `${base_path}${test_generator_path}metadata.json`,
                    `${archive_base_path}${test_generator_path}metadata.json`
                ),
                this.addFileFromGithubToArchive(
                    user,
                    exercise,
                    archive,
                    `${base_path}${test_generator_path}${test_generator.pathname}`,
                    `${archive_base_path}${test_generator_path}${test_generator.pathname}`
                )
            );
        }
        ...
        for (const test_set of exercise.test_sets) {
            ...
            asyncArchiveWriters.push(
                this.addFileFromGithubToArchive(
                    ...
                    exercise,
                    archive,
                    `${base_path}${test_set_path}metadata.json`,
                    `${archive_base_path}${test_set_path}metadata.json`
                )
            );
        }
        ...
        for (const test of exercise.tests) {
            ...
            asyncArchiveWriters.push(
                this.addFileFromGithubToArchive(
                    ...
                    exercise,
                    archive,
                    `${base_path}${test_path}metadata.json`,
                    `${archive_base_path}${test_path}metadata.json`
                ),
                this.addFileFromGithubToArchive(
                    user,
                    exercise,
                    archive,
                    `${base_path}${test_path}${test.input.pathname}`,
                    `${archive_base_path}${test_path}${test.input.pathname}`
                ),
                this.addFileFromGithubToArchive(
                    user,
                    exercise,
                    archive,
                    `${base_path}${test_path}${test.output.pathname}`,
                    `${archive_base_path}${test_path}${test.output.pathname}`
                )
            );
        }
    }
