...
export class TestSetSyncProcessor {
    ...
    public async onTestSetCreate(job: Job) {
        this.logger.debug(
            `[onTestSetCreate] Create test set in Github repository`
        );
        ...
        await this.repository.update(testset.id, { sha: res });
        this.logger.debug(
            '[onTestSetCreate] Test set created in Github repository'
        );
    }
    ...
    public async onTestSetUpdate(job: Job) {
        this.logger.debug(
            `[onTestSetUpdate] Update test set in Github repository`
        );
        const { user, testset } = job.data;
        const exercise = await this.exerciseService.findOne(
            testset.exercise_id
        ...
        await this.repository.update(testset.id, { sha: res });
        this.logger.debug(
            '[onTestSetUpdate] Test set updated in Github repository'
        );
    }
    ...
    public async onTestSetDelete(job: Job) {
        this.logger.debug(
            `[onTestSetDelete] Delete test set in Github repository`
        );
        const { user, testset } = job.data;
        const exercise = await this.exerciseService.findOne(
            testset.exercise_id
        ...
        this.logger.debug(
            '[onTestSetDelete] Test set deleted in Github repository'
        );
    }
}
