...
export class DynamicCorrectorController {

    constructor(
        protected readonly service: DynamicCorrectorService,
    ...
    async create(
        @User() user: any,
        @UploadedFile() file,
        @Body() dto: DynamicCorrectorEntity
    ): Promise<DynamicCorrectorEntity> {
    ...
    @Delete('/:id')
    async delete(@User() user: any, @Param('id') id: string): Promise<DynamicCorrectorEntity> {
        const accessLevel = await this.service.getAccessLevel(id, user.id);
        if (accessLevel < AccessLevel.CONTRIBUTOR) {
            throw new ForbiddenException(`You do not have sufficient privileges`);
        }
