...
export class DynamicCorrectorService {
    ...
    constructor(
        ...
        @InjectQueue(DYNAMIC_CORRECTOR_SYNC_QUEUE)
        private readonly dynamicCorrectorSyncQueue: Queue,
    ...
    public async getOne(
        ...
        try {
            ...
            throw new InternalServerErrorException(
                `Failed to get dynamic corrector`,
                e
            );
        }
    }
    ...
    public async createOne(
        user: UserEntity,
        dto: DynamicCorrectorEntity,
        file: any
    ): Promise<DynamicCorrectorEntity> {
        dto.pathname = file.originalname;
        try {
            const entity = await this.repository.save(
                plainToClass(DynamicCorrectorEntity, dto)
            );
            this.dynamicCorrectorSyncQueue.add(DYNAMIC_CORRECTOR_SYNC_CREATE, {
                user,
                entity
            });
            ...
            throw new InternalServerErrorException(
                `Failed to create dynamic corrector`,
                e
            );
        }
    }
    ...
    public async updateOne(
        ...
        try {
            const entity = await this.repository.save(
                plainToClass(DynamicCorrectorEntity, {
                    ...dynamic_corrector,
                    ...dto
                })
            );
            this.dynamicCorrectorSyncQueue.add(DYNAMIC_CORRECTOR_SYNC_UPDATE, {
                user,
                entity
            });
            this.dynamicCorrectorSyncQueue.add(
            ...
            throw new InternalServerErrorException(
                `Failed to update dynamic corrector`,
                e
            );
        }
    }
    ...
    public async deleteOne(
        ...
        try {
            ...
            throw new InternalServerErrorException(
                `Failed to delete dynamic corrector`,
                e
            );
        }
        this.dynamicCorrectorSyncQueue.add(DYNAMIC_CORRECTOR_SYNC_DELETE, {
            user,
            entity
        });
        return entity;
    ...
    public async getAccessLevel(
        ...
        const access_level = await getAccessLevel(
            [
                ...
                {
                    src_table: 'exercise',
                    dst_table: 'dynamic_corrector',
                    prop: 'dynamic_correctors'
                }
            ],
