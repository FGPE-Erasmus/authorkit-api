...
export class LeaderboardSyncProcessor {
    ...
    public async onLeaderboardCreate(job: Job) {
        this.logger.debug(
            `[onLeaderboardCreate] Create leaderboard in Github repository`
        );
        ...
        await this.repository.update(leaderboard.id, { sha: res });
        this.logger.debug(
            '[onLeaderboardCreate] Leaderboard created in Github repository'
        );
    }
    ...
    public async onLeaderboardUpdate(job: Job) {
        this.logger.debug(
            `[onLeaderboardUpdate] Update leaderboard in Github repository`
        );
        const { user, leaderboard } = job.data;
        const gamificationLayer = await this.gamificationLayerService.findOne(
            leaderboard.gl_id
        ...
        await this.repository.update(leaderboard.id, { sha: res });
        this.logger.debug(
            '[onLeaderboardUpdate] Leaderboard updated in Github repository'
        );
    }
    ...
    public async onLeaderboardDelete(job: Job): Promise<void> {
        this.logger.debug(
            `[onLeaderboardDelete] Delete leaderboard in Github repository`
        );
        const { user, leaderboard } = job.data;
        const gamificationLayer = await this.gamificationLayerService.findOne(
            leaderboard.gl_id
        ...
        this.logger.debug(
            '[onLeaderboardDelete] Leaderboard deleted in Github repository'
        );
    }
}
