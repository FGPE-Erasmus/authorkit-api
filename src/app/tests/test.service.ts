...
export class TestService {
    ...
    public async createTest(user: UserEntity, dto: TestEntity, input: any, output: any):
            Promise<TestEntity> {
        ...
        this.testSyncQueue.add(TEST_INPUT_SYNC_CREATE, {
            user, test, content: input
        }, { delay: 1000 });
        this.testSyncQueue.add(TEST_OUTPUT_SYNC_CREATE, {
            user, test, content: output
        }, { delay: 1500 });

        return test;
    }

    public async updateTest(user: UserEntity, id: string, dto: TestEntity, input: any, output: any):
            Promise<TestEntity> {
        ...
        this.testSyncQueue.add(TEST_INPUT_SYNC_UPDATE, {
            user, test, content: input
        }, { delay: 1000 });
        this.testSyncQueue.add(TEST_OUTPUT_SYNC_UPDATE, {
            user, test, content: output
        }, { delay: 1500 });
    ...
    public async deleteTest(user: UserEntity, id: string):
            Promise<TestEntity> {
    ...
    public async importProcessEntries(
        user: UserEntity, exercise: ExerciseEntity, entries: any, testset?: TestSetEntity
    ): Promise<void> {
        ...
        const entity = await this.importMetadataFile(user, exercise, root_metadata, testset);
        ...
        this.testSyncQueue.add(
            TEST_INPUT_SYNC_CREATE,
            {
                user, test: entity, content: { buffer: (await entries[entity.input.pathname].buffer()) }
            },
            { delay: 1000 }
        );
        ...
        this.testSyncQueue.add(
            TEST_OUTPUT_SYNC_CREATE,
            {
                user, test: entity, content: { buffer: (await entries[entity.output.pathname].buffer()) }
            },
            { delay: 1500 }
        );
    }
    ...
    public async importMetadataFile(
        user: UserEntity, exercise: ExerciseEntity, metadataFile: any, testset?: TestSetEntity
    ): Promise<TestEntity> {
        ...
        this.testSyncQueue.add(
            TEST_SYNC_CREATE, { user, test: entity }
        );
